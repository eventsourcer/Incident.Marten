...
public class IncidentShortInfoProjection : SingleStreamProjection<IncidentShortInfo>
{
    ...
    public IncidentShortInfo Apply(AgentRespondedToIncident responded, IncidentShortInfo current) =>
        current with { NotesCount = current.NotesCount + 1 };

    public IncidentShortInfo Apply(CustomerRespondedToIncident responded, IncidentShortInfo current) =>
        current with { NotesCount = current.NotesCount + 1 };
